...
export function CPButton({
  ...
  styles = {},
  ...otherProps
}: CubeButtonProps & { variant?: keyof typeof CPButtonVariantProps } & { openInNew?: boolean }) {
  const CPButtonVariantProps = {
    ...
    danger: {
      ...
      fill: {
        ...
        disabled: 'var(--cp-surface-critical-subdued)',
      },
  ...
  return (
    <Button {...sharedButtonStyles} {...CPButtonVariantProps[variant]} {...otherProps}>
      {children}
    </Button>
  );
}
...
export function CPButtonOpenInNew(props: CubeButtonProps) {
  return (
    <Button type="link" to={`!${props.to}`} outline="0">
      <Flex style={{ whiteSpace: 'normal', gap: '0.5rem', alignItems: 'center' }}>
        {props.children}
...
export function CPSpinner() {
  ...
  return (
    <CPRow>
      <svg xmlns="http://www.w3.org/2000/svg" width="40" height="38" viewBox="0 -50 40 300">
        <g className="segment">
          <path
            id="loading-path"
